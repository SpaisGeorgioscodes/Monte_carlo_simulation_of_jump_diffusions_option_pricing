%Path generator
%   This script creates and plots the paths
% For the following paraments
 strart_date=0;
end_date=1060;
s0=100;
sigma=0.2;
r=0.01;
num_paths=50;
% Parametroi toy modelou 
lamda=0.8;
a=0.05; % mean of normal distribution (
b=0.15; % standar deviation of normal distribution
m=exp(b^2/2+a);  % mean of the lognormal distribution  of the jump
v=exp(2*a+b^2)*(exp(b^2)-1); % variance of the lognormal distribution  of the jump

% m=0.9;
% v=0.4;
% a=log(m^2/sqrt(v+m^2));
% b=log(v/(m^2)+1);
num_steps=end_date-strart_date;
dt=(end_date-strart_date)/(num_steps*252);
t=(end_date-strart_date)/(252);

x=zeros(num_paths, num_steps);


%paths creation
for i=1:num_paths
    x(i,1)=log(s0);
    M=0;
    N=0;
    
    for j=2:num_steps
        N=random('Poisson',lamda*dt);
        M=a*N+b*sqrt(N)*normrnd(0,1);
        
             x(i,j)=x(i,j-1)+(r-lamda*(m-1)-0.5*sigma*sigma)*dt+sigma*normrnd(0,1)*sqrt(dt)+M;
    end
end
sample=exp(x);

time=zeros(1, num_steps);
time(1,1)=0;
for i=2:num_steps
    time(1,i)=(i-1)*dt;
end;


for i=1:num_paths
    plot(time(1,:), sample(i,:), 'color',rand(1,3));
    hold on;
end
hold off